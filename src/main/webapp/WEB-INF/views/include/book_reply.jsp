...
<form id="replyForm" name="replyForm" method="POST" class="replyWrite">
	<input type="hidden" id="bno" name="bno" value="${bookBean.no}" />
...
<!--  댓글 내용 -->
			<div id="reply">
				<ol class="replyList">
					<c:forEach var="bookReplyBean" items="${bookReplyList}"
													begin="0" 
													end="${bookReplyList.size() }" 
													step="1"
													varStatus="status">
						<li>
							<div class="reply">
								<img src="${bookReplyBean.profileImg}" class="profileImg">
								<div class="replytest">
								<p class="writer">작성자 : ${bookReplyBean.writer} </p>
								<p class="numbering"> ${numbering - status.index} </p> 
								<p class="content">${bookReplyBean.content}</p>
								<p class="regDate"><fmt:formatDate value="${bookReplyBean.regDate}" pattern="yyyy-MM-dd[E] a hh:mm:ss" /></p>
								</div>
								<c:set var="a" value="${loggedMemberInfo.id}" scope="session"/>
								<c:set var="b" value="${bookReplyBean.writer}" scope="session"/>
								<c:if test="${a eq b || a == 'admin'}">
									<button type="button" class="replyDeleteBtn btn btn-danger"
									onclick="location.href='BookReplyDelete.do?rno=${bookReplyBean.rno}&no=${bookReplyBean.bno}'">삭제</button>
									<button type="button" class="replyUpdateBtn btn btn-warning"
									onclick="location.href='BookReplyUpdateForm.do?rno=${bookReplyBean.rno}'"data-rno="${replyList.rno}">수정</button>
...
<!--  		                            게시판 페이징                                     -->	
			<div class="pagination">
				...
				<c:if test="${startPage > pageGroup }">
					<button onclick="location.href='BookInfo.do?no=${bno}&clickedPage=1'">
						<span>처음</span>
					</button>
					<a href="BookInfo.do?no=${bno}&clickedPage=${startPage - pageGroup}">
						<span>이전</span>
					</a>
				</c:if>	
				
				<c:forEach var="i" begin="${startPage}" end="${endPage}" varStatus="status">
					<a href="BookInfo.do?no=${bno}&clickedPage=${i}" class="btn btn-info">${i}</a>
				</c:forEach>
				...
				<c:if test="${endPage < paginationTotal }">
					...
					<button onclick="location.href='BookInfo.do?no=${bno}&clickedPage=${paginationTotal }'">
						<span>끝</span>
					</button>
				</c:if>
			</div>
